...
echo("<head>");
echo("<title>$menu_title - $um_title</title>");
				...
				<?php
						...
						echo"
							<FORM name='add' method='post' action='" . $_SERVER['PHP_SELF'] . "'>
								...
								<p>
									...
									<select name='status'>
									<option value='0'>$um_inactive</option>
									<option value='1'>$um_active</option>
									</select>
									</label>
								</p>
								<p>
					...
					elseif(isset($_POST['add'])) {
						...
						if (isset ($User) && isset($status) && isset($Password) && isset ($Uid) && isset ($Gid) && isset ($Dir) && isset ($DLBandwidth) && isset ($ULBandwidth) && isset ($ipaccess) && isset ($QuotaSize) && isset ($QuotaFiles)) {
							$result = mysqli_query($db,"INSERT INTO ftpd (User,status,Password,Uid,Gid,Dir,ULBandwidth,DLBandwidth,ipaccess,QuotaSize,QuotaFiles) VALUES ('$User','$status',md5('$Password'),'$Uid','$Gid','$Dir','$ULBandwidth','$DLBandwidth','$ipaccess','$QuotaSize','$QuotaFiles')");
					...
					elseif(isset($_POST['edit'])) {
						...
						$result = mysqli_query($db,"SELECT * FROM ftpd WHERE id=$id");
						$array = mysqli_fetch_array ($result);

						// Проверяем были ли внесены какие-то изменения
						if (($Uid != $array['Uid']) || ($Gid != $array['Gid']) || ($Dir != $array['Dir']) || ($User != $array['User']) || ($status != $array['status']) || (md5($Password) != $array['Password']) || ($ULBandwidth != $array['ULBandwidth']) || ($DLBandwidth != $array['DLBandwidth']) || ($ipaccess != $array['ipaccess']) || ($QuotaSize != $array['QuotaSize']) || ($QuotaFiles != $array['QuotaFiles'])) {

							// Uid
							if (($Uid != $array['Uid']) && isset ($id)) {
								$result = mysqli_query($db,"UPDATE ftpd SET Uid='$Uid' WHERE id='$id'");
							...
							if (($Gid != $array['Gid']) && isset ($id)) {
								$result = mysqli_query($db,"UPDATE ftpd SET Gid='$Gid' WHERE id='$id'");
							...
							if (($Dir != $array['Dir']) && isset ($id)) {
								$result = mysqli_query($db,"UPDATE ftpd SET Dir='$Dir' WHERE id='$id'");
							...
							if (($User != $array['User']) && isset ($id)) {
								$result = mysqli_query($db,"UPDATE ftpd SET User='$User' WHERE id='$id'");
							...
							if (($status != $array['status']) && isset ($id)) {
								$result = mysqli_query($db,"UPDATE ftpd SET status='$status' WHERE id='$id'");
							...
							if (isset ($Password)) {
								...
								if (($Password != $array['Password']) && isset ($id)) {
									$result = mysqli_query($db,"UPDATE ftpd SET Password='$Password' WHERE id='$id'");
							...
							if (($ULBandwidth != $array['ULBandwidth']) && isset ($id)) {
								$result = mysqli_query($db,"UPDATE ftpd SET ULBandwidth='$ULBandwidth' WHERE id='$id'");
							...
							if (($DLBandwidth != $array['DLBandwidth']) && isset ($id)) {
								$result = mysqli_query($db,"UPDATE ftpd SET DLBandwidth='$DLBandwidth' WHERE id='$id'");
							...
							if (($ipaccess != $array['ipaccess']) && isset ($id)) {
								$result = mysqli_query($db,"UPDATE ftpd SET ipaccess='$ipaccess' WHERE id='$id'");
							...
							if (($QuotaSize != $array['QuotaSize']) && isset ($id)) {
								$result = mysqli_query($db,"UPDATE ftpd SET QuotaSize='$QuotaSize' WHERE id='$id'");
								if ($result == 'true') {
							...
							if (($QuotaFiles != $array['QuotaFiles']) && isset ($id)) {
								$result = mysqli_query($db,"UPDATE ftpd SET QuotaFiles='$QuotaFiles' WHERE id='$id'");
								if ($result == 'true') {
					...
					echo"</br>
						...
						$result = mysqli_query($db,"SELECT * FROM ftpd");
					...
					while ($myrow = mysqli_fetch_array ($result));
						...
						echo("<table width='95%' border='0' align='center' cellpadding='0' cellspacing='0'><tr><td align='right'>
					...
					// Эта часть используется, если выбран пользователь для редактирования
						...
						$myrow = mysqli_fetch_array ($result);
						if ($myrow['status'] == 0) {
							$select = "<option value='0' selected='selected'>$um_inactive</option><option value='1'>$um_active</option>";
						}
						else {
							$select = "<option value='0'>$um_inactive</option><option value='1' selected='selected'>$um_active</option>";
						}
						print("
